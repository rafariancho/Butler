...
namespace Butler.Factories
{
    public class WeeklyMenuFactory : IWeeklyMenuFactory
    {
        ...
        public List<DailyMenu> GetWeeklyMenu()
        {
            ...
            if (_repository.GetDishesCount(Models.Type.Lunch) >= 7)
            {
            ...
            for (int k = 0; k < 7; k++)
            {
                ...
                if (menus[k] == null) {
                    menus[k] = new Menu() { Lunch = new Dish() { Id = -1, Name = "Not enough dishes yet" }, Dinner = new Dish() { Id = -1, Name = "Not enough dishes yet" } };
                }
